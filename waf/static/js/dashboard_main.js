...
function setupEventListeners() {
    ...
    const mlValueOut1 = document.getElementById('threshold-value1');
    const mlValueOut2 = document.getElementById('threshold-value2');
    if (mlSlider) {
        mlSlider.addEventListener('input', function () {
            const value = parseFloat(this.value).toFixed(1);
            if (mlValueOut1) mlValueOut1.textContent = value;
            if (mlValueOut2) mlValueOut2.textContent = value;
        });
    }
    // Blocked only filter
...
function populateSettingsForm(settings) {
    ...
    // Active model info (added by backend)
    const activeCountEl = document.getElementById('active-model-count');
    const activeNamesEl = document.getElementById('active-model-names');
    const detectionStatusEl = document.getElementById('detection-status');
    const detectionStatusCardEl = document.getElementById('detection-status-card');
    const modelNameDisplayEl = document.getElementById('model-name-display');
    
    if (activeCountEl) activeCountEl.textContent = settings.active_model_count || 0;
    if (activeNamesEl) {
        const names = (settings.active_models || []).map(m => m.name).join(', ') || 'None';
        activeNamesEl.textContent = names;
    }
    if (detectionStatusEl) detectionStatusEl.textContent = (settings.ml_model && settings.ml_model.enabled) ? 'Active' : 'Disabled';
    if (detectionStatusCardEl) detectionStatusCardEl.textContent = (settings.ml_model && settings.ml_model.enabled) ? 'Active' : 'Disabled';
    if (modelNameDisplayEl) {
        const modelName = (settings.active_models || []).map(m => m.name).join(', ') || 'No model loaded';
        modelNameDisplayEl.textContent = modelName;
    }
...
function displayRules(rules) {
    ...
    allRules = JSON.parse(JSON.stringify(rules));
    
    const container = document.getElementById('rules-editor');

    if (rules.length === 0) {
        container.innerHTML = '<div class="text-center text-muted p-4">No rules found. <a href="#" onclick="addNewRule(); return false;">Add a new rule</a></div>';
        return;
    }
...
// Global variable to track all rules
let allRules = [];

// Add new rule - Opens Modal properly
function addNewRule() {
    // Reset form
...
// Edit rule - Opens Modal with existing data
function editRule(id, pattern, action, description) {
    document.getElementById('rule-id').value = id;
    document.getElementById('rule-id').setAttribute('readonly', 'readonly');
    document.getElementById('rule-pattern').value = pattern;
    document.getElementById('rule-action').value = action;
    document.getElementById('rule-description').value = description;
...
function saveRule() {
    ...
    if (!ruleId || !pattern || !action || !description) {
        showNotification('Please fill in all fields', 'error');
        return;
    }

    // Check if editing existing rule or adding new
    const existingRuleIndex = allRules.findIndex(r => r.id === ruleId);
    
    if (existingRuleIndex > -1) {
        // Update existing rule
        allRules[existingRuleIndex] = {
            id: ruleId,
            pattern: pattern,
            action: action,
    ...
    fetch('/api/rules', {
        ...
        .then(data => {
            if (data.success) {
                ...
                showNotification(data.error || 'Error saving rules', 'error');
            }
        })
        .catch(error => {
...
// Save rules (backward compatibility)
function saveRules() {
    saveAllRules();
}
