...
# Graceful MongoDB wrapper
class MongoLoggerWrapper:
    """Wrapper that gracefully handles MongoDB unavailability"""
    def __init__(self):
        try:
            self._logger = MongoLogger()
            self._available = True
            print("[Dashboard] MongoDB connection successful")
        except Exception as e:
            print(f"[Dashboard] Warning: MongoDB not available: {e}")
            self._logger = None
            self._available = False
    
    def __getattr__(self, name):
        if not self._available:
            # Return a dummy object that returns empty lists/dicts
            return lambda *args, **kwargs: [] if name in ['find', 'aggregate'] else None
    ...
    def collection(self):
        ...
        return self._logger.collection
                    ...
                    class DummyDB:
                        def __getitem__(self, key):
                            class DummyCollection:
                                ...
                                def sort(self, *args, **kwargs): return []
                            return DummyCollection()
                    return DummyDB()
            return DummyClient()
        return self._logger.client
    
    def is_ip_blocked(self, ip):
        if not self._available:
            return False
        return self._logger.is_ip_blocked(ip)
    
    def block_ip(self, ip, duration):
        if not self._available:
            return
        return self._logger.block_ip(ip, duration)
    
    def increment_request_count(self, ip, window):
        if not self._available:
            return 0
        return self._logger.increment_request_count(ip, window)
...
mongo_logger = MongoLoggerWrapper()
...
try:
    if os.path.exists(_TEXT_MODEL_PATH):
        _text_predictor = WafPredictor(_TEXT_MODEL_PATH)
        print(f"[Dashboard] ML Model loaded: {_TEXT_MODEL_PATH}")
    else:
        print(f"[Dashboard] No text model found at {_TEXT_MODEL_PATH}")
except Exception as e:
    print(f"[Dashboard] WafPredictor initialization failed: {e}")
...
def manage_settings():
    ...
    # Build response with active model information
    resp = dict(WAF_SETTINGS)
    active = []
    try:
        if _text_predictor is not None:
            name = getattr(_text_predictor, 'model_display_name', None) or getattr(_text_predictor, 'model_name', None) or 'text_model'
            active.append({'name': name})
    except Exception as e:
        print(f"[Dashboard] Error getting model info: {e}")
    
    resp['active_models'] = active
    resp['active_model_count'] = len(active)
    return jsonify(resp)
...
def get_rules():
    ...
    try:
        ...
        if rules_data is None:
            rules_data = {'rules': []}
        elif not isinstance(rules_data, dict):
            rules_data = {'rules': []}
        ...
        print(f"[Dashboard] Error in get_rules: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'error': str(e)}), 500
...
def update_rules():
    ...
    try:
        ...
        print(f"[Dashboard] Received request to update rules with {len(rules)} rules")
        
        # Validate rules structure
        for rule in rules:
            if not all(key in rule for key in ['id', 'pattern', 'action', 'description']):
                print(f"[Dashboard] Invalid rule structure: {rule}")
                return jsonify({'error': 'Invalid rule structure'}), 400
            
            if rule['action'] not in ['block', 'log']:
                print(f"[Dashboard] Invalid action: {rule['action']}")
        ...
        print(f"[Dashboard] Successfully saved {len(rules)} rules to {rules_file_path}")
        ...
        print(f"[Dashboard] Error updating rules: {e}")
        return jsonify({'error': str(e)}), 500
