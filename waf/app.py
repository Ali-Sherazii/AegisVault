...
try:
    ...
    print(f"[WAF] Warning: MongoDB not available: {e}")
...
try:
    if os.path.exists(_TEXT_MODEL_PATH):
        ...
        print(f"[SUCCESS] ML Model loaded: {_TEXT_MODEL_PATH}")
    else:
        print(f"[WARNING] Model file not found: {_TEXT_MODEL_PATH}")
except Exception as e:
    print(f"[ERROR] WafPredictor initialization failed: {type(e).__name__}: {e}")
    import traceback
    traceback.print_exc()
...
def manage_settings_api():
    ...
    Writes directly to the same `waf_settings.json` used by this app.
    """
    if request.method == 'POST':
        data = request.get_json(silent=True) or {}
        # Merge incoming settings with existing ones to preserve unspecified keys
        current = load_waf_settings()
        current.update(data)
        try:
            with open(SETTINGS_FILE, 'w') as f:
                json.dump(current, f, indent=2)
            print(f"[WAF] Settings updated via API: {SETTINGS_FILE}")
            return jsonify({'success': True, 'message': 'Settings updated successfully'})
        except Exception as e:
            print(f"[WAF] Failed to save settings: {e}")
            return jsonify({'success': False, 'error': str(e)}), 500

    # GET - return the current settings plus active model info
    resp = load_waf_settings()
    # Attach active model info if available
    active = []
    try:
        if _text_predictor is not None:
            name = getattr(_text_predictor, 'model_display_name', None) or getattr(_text_predictor, 'model_name', None) or getattr(_text_predictor, 'model_path', None) or 'text_model'
...
def waf_filter():
    ...
    # 3. Evaluate rules first (reload rules for each request to pick up dashboard updates)
    if settings.get('rules', {}).get('enabled', True):
        try:
            # Create a fresh RuleEngine instance for each request to load latest rules
            rule_engine = RuleEngine("rules.yaml")
            print(f"[WAF] Loaded {len(rule_engine.rules)} rules from rules.yaml")
    ...
    if not blocked:
        ...
        # Enforce configured confidence threshold for all prediction modes
        if ml_enabled and prediction_result and prediction_result.is_malicious and (prediction_result.confidence >= confidence_threshold):
            blocked = True
