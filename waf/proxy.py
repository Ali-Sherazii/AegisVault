...
try:
    if os.path.exists(_TEXT_MODEL_PATH):
        ...
        print(f"[WAF-PROXY] ML Model loaded: {_TEXT_MODEL_PATH}")
    else:
        print(f"[WAF-PROXY] No text model found at {_TEXT_MODEL_PATH}; using feature extractor fallback")
except Exception as e:
    print(f"[WAF-PROXY] WafPredictor initialization failed: {e}")
...
def manage_settings_api():
    """Allow settings to be fetched/updated via the proxy app as well.
    This writes to the same waf_settings.json so changes are visible to other processes.
    """
    # Compute SETTINGS_FILE path already defined above
    if request.method == 'POST':
        ...
        try:
            ...
            print(f"[WAF-PROXY] Settings updated via API: {SETTINGS_FILE}")
            return jsonify({'success': True, 'message': 'Settings updated successfully'})
        except Exception as e:
            print(f"[WAF-PROXY] Failed to save settings: {e}")
...
def waf_proxy(path):
    ...
    if ml_enabled and prediction and prediction.is_malicious and (prediction.confidence >= confidence_threshold):
